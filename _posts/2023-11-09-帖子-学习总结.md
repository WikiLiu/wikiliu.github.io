...
>* dpkg -l 
>* sudo apt show
>* gdb [工具参考](https://linuxtools-rst.readthedocs.io/zh-cn/latest/tool/gdb.html) call（函数 b函数） until（跳出 终止） catch(捕获系统调用) watch（改变就打印值） backtrace  up/dowm
>* ps aux ef
>* 
## FromWiki
* /proc/driver/dri0
...
* io remap:__iomem 是一个宏定义，表示这段内存是用于 I/O 操作的，编译器会根据它来生成相应的代码，确保读写操作的正确性。
...
## legacy建立pipeline ![](../assets/images/KMS_common.png)

* drm_mode_card_res   drm_mode_get_plane_res  drm_mode_get_connector  drm_mode_modeinfo  drm_mode_get_encoder  简单点就是拿到crtc（硬件）选择plane（哪个）
* drm_mode_crtc
